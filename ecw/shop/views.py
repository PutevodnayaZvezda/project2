...
def search(request):
    ...
    if len(allprods) == 0 or len(query)<4:
        params = {'msg' : "Пожалуйста, обязательно введите соответствующий поисковый запрос"}
    return render(request,'shop/index.html', params)
...
def checkout(request):
    if request.method =="POST":
        ...
        checkout.save()
        update = OrderUpdate(order_id=checkout.order_id,update_desc="Заказ был размещен.")
        update.save()
        thank = True
        id=checkout.order_id
        param_dict ={
            'MID': 'Ваш-Идентификатор-продавца-Здесь',
...
def handlerequest(request):
    ...
    if verify:
        if response_dict['RESPCODE'] == '01':
            ...
            print('заказ не выполнен, потому что ' + response_dict['RESPMSG'])
